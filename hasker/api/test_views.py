...
class ViewsTest(APITestCase):
    ...
    def test_search_page_good_phrase(self):
        url = "{}?={}".format(reverse("api:question:search"), "question")
        response = self.client.get(url, format="json")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response['content-type'], 'application/json')
        self.assertEqual(response.data["count"], 1)
    ...
    def test_search_page_bad_phrase(self):
        url = "{}?={}".format(reverse("api:question:search"), "bad_phrase")
        response = self.client.get(url, format="json")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response['content-type'], 'application/json')
        self.assertEqual(response.data["count"], 1)
    ...
    def test_detail_page_exist_id(self):
        ...
        self.assertEqual(
            ...
            DateTimeField().to_representation(self.test_question.created)
        )
    ...
    def test_answers_page_exist_id(self):
        ...
        self.assertEqual(
            ...
            DateTimeField().to_representation(self.test_answer.created)
        )
